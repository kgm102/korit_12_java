...
/*
    방금 작성한 ProdectView와 인터넷에서의 레퍼런스를 참조하여
    Samsung 클래스의 싱글톤 패턴으로 작성하시오.
 */
public class Samsung {
    ...
    private int seriealNumber;

    // 생성자
    private Samsung(){
        company = getClass().getSimpleName();   // Samsung
        seriealNumber = 20260000;               // 정적 변수로 20260000으로 만들면
    ...
    public String createSerialNumber(String model) {
        return model + "_" + ++seriealNumber;
    }
    /*
        이미 싱글톤을 배운 우리는 어차피 객체가 하나만 생성될거라는 것을 알고 있는 상태입니다.
        근데 이 메서드를 호출했을 때, ++가 적용은 되겠죠. 그러면 다수의 객체를 만들려고 시도해봤자 하나만
        만들어지기 때문에 모델명_20260000 에서 1씩 증가된 값이 시리얼 넘버가 될 것이고,
