...
1. 클래스란 ?
    ...
    Object와 Intense는 우리나라에서는 거의동일한 용어로 사용됩니다.

    객체와 인스턴스 관계
        객체는 포괄적인 개념으로 클래스에서 생성된 모든 것을 객체라고 표현
        인스턴스는 특정 객체가 어떤 클래스의 구현체인지 강자호는 표현 비유로 이해하기
        비유로 이해하기
        1. "사람"이라는 클래스가 있다고 가정한다면,
        2."홍길동", "김철수"는 이 클래스에서 생성된 객체임. 근데 "아반떼"도 객체임
        3."홍길동"은 "사람"이라는 특정 클래스의 '인스턴스'음. 아반ㅖ는 아님.
 */
public class ClassA {
    ...
    String name;
    double score;
    // 이상의 코드에서 보시면 선언만 되어있고 초기화는 이루어지지 않았았습니다.

    //메서드 정의 방법
    //접근제어자 리턴타입 메서드면(매개변수, 매개변수2){
    // 실행문()
    //  (return)
    // }
