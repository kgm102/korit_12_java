...
public class Main {
    public static void main(String[] args) {
        Dog dog1 = new Dog();
        dog1.makeSound();   //  개가 짓습니다.
        dog1.fetch();       // 강아지가 공을 물어옵니다.

        // 업캐스팅 # 1
        Animal animal1 = dog1;
        animal1.makeSound();    // 개가 짓습니다. 즉, 재정의된 method가 호출됐습니다.
//        animal1.fetch(); // 불가능

        // 업캐스팅 # 2
        Animal animal2 = new Dog(); // 이게 어제 말한 A a = new B(); 형태입니다. 시험에 자주 나옴.
        animal2.makeSound();    // 개가 짓습니다. 즉, 재정의된 method가 호출됐습니다.
//        animal2.fetch(); // 불가능
        /*
            ...
            그럼 재정의도미 method 버전으로 호출이 될거라면 애초에
        ...
        /*
            ...
            근데 여기서 중요한 점은 다운캐스팅ㅇ르 했다고 해서 animal2가 사라지는 것은 아닙니다.
            dog2와 animal2가 별개로 움직인다고 봐야합니다.
            dog2로 메서드를 호출할 경우 당연히 재정의된 makeSound()가 출력되며,
            고유 메서드인 fetch() 역시 호출이 가능합니다.

            그러면 Dog dog0 = new Dog();로 만듦녀 애초에 재정의된 메서드 및 고유 메서드를 사용할 수 있다는
            점이 문제가 됩니다.
         */
...
//        dog3.makeSound();
        /*
            이상의 경우를 확인했을 때 dog3.makeSound();로 가기 전에 이미 downcasting 부분에서(77버 번 라인) 오류가
            발생했음을 확인할 수 있습니다.
            즉 애초에 animal3를 생성할 때 Animal클래스의 생성자를 호출해서 만들었기 때문에
            (A a = new A(); 이기  때문에)
            하위 클랫의 인스턴스로 다운캐스팅하는 것이 불가능합니다.
         */
        Animal animal4 = new Dog();
        // 와 같은 방식으로 선언된 클래스와 호출된는 생성자가 서로 달라야 합니다(그리고 둘은 상속과계여야만 합니다.).

        // 근데 문제는 77번  라인에서 빨전 줄 미리 띄워서 오류가 발생할 예정이라고 우리에게 알려주지 않는다는 점입니다.
