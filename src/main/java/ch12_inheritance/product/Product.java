...
/*
    ...
    왜 오류가 발생하는지, 어떻게 검증하는지 어려움을 겪을 수 있ㅅ브니다.

    부모-자식의 상속 관계가 성립되어 있을 때,
    부모 클래스에서 기본 생성자 없이 생성자만 있다면,
    자식 클래스에서 완벽한 기본 생성자를 만드는 긋이 '불가능'합니다.

    자식 클래스의 객체를 생성 시 필수적으로 '부모 클래스의 생성자를 호출'하기 때문입니다.
    ...
    A a = new B(); 같은 방식으로 생성하는 내용이 있을 겁니다. -> 정치기 단골문제
 */
